//NOTE
/*
-để chạy chương trình dùng lệnh "make" trong terminal 
-SAU KHI SỬA XONG CODE CHẠY LỆNH "make clean" để recomile chương trình
...
#include "bai2/Distribution.h"
#include "path/voronoid.h"
#include "pedestrian/Crowd.h"
using namespace std;
...
std::map<std::string, std::vector<float>> mapData2;
...
// can thay do lai cach tao toa do cac buc tuong
...
void setAgentsFlow(Agent *agent, float desiredSpeed, float maxSpeed, float minSpeed, int caseJump)
{
    ...
    //codesrc: 0 From Left, 1 From Right, 2 From Top, 3 From Bottom
    //codedes:0 To Right, 1 To Left, 2 To Bottom, 3 To Top
...
void createAgents()
{
    ...
    vector<double> velocityList = pedes.getVelo_dist();
...
void update()
{
    ...
    for (AGV *agv : agvs)
    {
        ...
        {
            agv->setCollisionStartTime(glutGet(GLUT_ELAPSED_TIME));//lay moc tgian
