...
//readmapdata updated:
std::map<std::string, std::vector<float>> Utility::readMapData2(
    const char *fileName)
{
   std::map<std::string, std::vector<float>> map;
    ...
    int numI;
    input >> numI;
    map["numI"].push_back(numI);

    for (int i = 0; i < numI; ++i) {
        float entryX, entryY, exitX, exitY, width;
        char abbrev;
        input >> entryX >> entryY >> exitX >> exitY >> width >> abbrev;
        std::vector<float> data;
        data.push_back(entryX);
        data.push_back(entryY);
        data.push_back(exitX);
        data.push_back(exitY);
        data.push_back(width);
        map[std::string(1, abbrev)] = data;
    }

    float aEntry1X, aEntry1Y, aEntry2X, aEntry2Y, aExit1X, aExit1Y, aExit2X, aExit2Y;
    input >> aEntry1X >> aEntry1Y >> aEntry2X >> aEntry2Y >>aExit1X>>aExit1Y>>aExit2X>>aExit2Y;
    std::vector<float> aData;
    aData.push_back(aEntry1X);
    aData.push_back(aEntry1Y);
    aData.push_back(aEntry2X);
    aData.push_back(aEntry2Y);
    aData.push_back(aExit1X);
    aData.push_back(aExit1Y);
    aData.push_back(aExit2X);
    aData.push_back(aExit2Y);
    map["A"] = aData;

    float s1x, s1y, s2x, s2y;
    input >> s1x >> s1y >> s2x >> s2y;
    std::vector<float> sData = {s1x, s1y, s2x, s2y};
    map["S"] = sData;

    float d1x, d1y, d2x, d2y;
    input >> d1x >> d1y >> d2x >> d2y;
    std::vector<float> dData = {d1x, d1y, d2x, d2y};
    map["D"] = dData;

    float mapCoords[8];
    for (int i = 0; i < 8; ++i)
        input >> mapCoords[i];
    std::vector<float> mapData(mapCoords, mapCoords + 8);
    map["mapCoords"] = mapData;
    return map;
}
...
// get list velocity of all pedestrians: type 0 - , type 1 
// - T distribution
std::vector<double> Utility::getPedesVelocity(int type, json inputData,std::vector<int>&group)
{
    if (type == 0)
    {
        return getPedesVelocityBasedDDis(group);
    }
    else
    {
...
std::vector<double> Utility::getPedesVelocityBasedDDis(std::vector<int>&group)
{
    vector<double> v;
        ...
        for(int j=0;j<group[4];j++){
            v.push_back(V5);
        }
        for(int j=0;j<group[5];j++){
            v.push_back(V6);
        }
...
//Hàm tạo vận tốc cho n người theo pp deu lien tuc 
