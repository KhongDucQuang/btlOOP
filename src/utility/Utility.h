...
namespace Utility
{
    ...
    std::map<std::string, std::vector<float>> readMapData2(const char *fileName);
    ...
    std::vector<double> getPedesVelocity(int type, json inputData,std::vector<int>&group);

    std::vector<double> getPedesVelocityBasedDDis(std::vector<int>&group);
